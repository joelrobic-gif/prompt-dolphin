...
export function buildHtmlDesignContract(brandTokens) {
  ...
  return [
    `Semantic HTML page (single self-contained <!doctype html> document).${ds}`,
    '',
    'STRUCTURE:',
    '  <head>: charset, viewport, <title>, all CSS inline in <style>.',
    '  <body>: <header> hero | <main> stacked sections | <footer> with as-of timestamp + source list.',
    '  Every numeric claim or quote has an inline <a href="..."> citation linked to source URL.',
    '',
    'TYPOGRAPHY:',
    `  Headings font-family: ${b.fontStack.sans}.`,
    `  Body font-family: ${b.fontStack.sans}.`,
    `  Monospace (data, code, tickers): ${b.fontStack.mono}.`,
    '  Body 17px / line-height 1.6 / max-width 760px content column for readability.',
    '  H1 32-40px bold; H2 24-28px semibold; H3 20px medium.',
    '',
    'PALETTE (light scheme — prefers-color-scheme aware via media query):',
    `  --accent: ${b.palette.primaryLight};  --accent-dark: ${b.palette.primaryDark};`,
    `  --ink: ${b.palette.inkLight};         --paper: ${b.palette.paperLight};`,
    `  --ink-dark: ${b.palette.inkDark};     --paper-dark: ${b.palette.paperDark};`,
    '  AAA contrast required for body text. Accent reserved for links + section markers.',
    '',
    'LAYOUT POLISH:',
    '  Hero: 60vh, accent gradient OR real source-attributed image. Headline + dek + as-of + 30-word TLDR.',
    '  Story cards: subtle 1px border in --ink at 12% alpha. Generous padding (24-32px). Soft 8px radius.',
    '  Section dividers: thin horizontal rule in --accent at 20% alpha.',
    '  Tables: striped rows in --ink at 4% alpha. Right-aligned numerics. Monospace for numbers.',
    '',
    'SOURCES POLICY:',
    '  Every numeric claim, quote, or named outlet carries an inline citation linking to the source URL.',
    '  Footer must contain a numbered <ol class="sources"> with publication + date + URL for each citation.',
    '  If you cannot cite a fact, OMIT it. Do NOT fabricate URLs, outlets, or quotes.',
    '',
    'NO:',
    '  No external <link rel="stylesheet"> imports (everything inline).',
    '  No external <script> tags (static page only).',
    '  No marketing prose ("revolutionary", "game-changing", "groundbreaking").',
    '  No invented quotes, statistics, outlets, or images.',
    '',
    `BRAND: this report is published by "${b.productName}".${b.logoSvgRef ? ` Include the brand mark via <img src="${b.logoSvgRef}" alt="${b.productName}"> top-left.` : ' Use a clean wordmark of the brand name top-left in headings font.'}`,
  ].join('\n');
}
